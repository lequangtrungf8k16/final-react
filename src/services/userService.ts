...
export const userService = {
  ...
  getProfile: () => {
    return api.get<{ data: User }>("/api/users/profile");
  },

  // Lấy thông tin user khác theo ID
  getUserById: (userId: string) => {
    return api.get<{ data: User }>(`/api/users/${userId}`);
  },

  // Cập nhật thông tin profile
  updateProfile: (data: FormData) => {
    return api.patch<{ data: User }>("/api/users/profile", data);
  },

  // Tìm kiếm user
  searchUsers: (query: string) => {
    return api.get<{ data: User[] }>("/api/users/search", {
      params: { q: query },
    });
  },
